<template>
      ...
      <n-table>
        ...
        <tbody>
          <tr v-for="(product, idx) in products" :key="product._id">
            ...
            <td><n-switch v-model:value="product.sell" @change="submitSell" /></td>
  ...
  <n-modal id="PDmodal" v-model:show="form.showModal" class="custom-card" preset="card" title=" " size="small" :bordered="false">
    ...
    <n-form ref="valid" :model="form" label-placement="top" @submit.prevent="submit" :rules="rules">
      <n-form-item label="商品名稱" path="name">
        <n-input v-model:value="form.name" placeholder=" " maxlength="30" show-count clearable />
      </n-form-item>
      ...
      <n-form-item label="商品價格" path="price">
        <n-input v-model:value="form.price" placeholder=" " type="number">
          <template #suffix>元</template>
        </n-input>
      </n-form-item>

      <n-form-item label="商品數量" path="quantity">
        <n-input-number
